...
if(mMenu){
  mMenu.querySelectorAll('a').forEach(function(a){
    a.addEventListener('click',function(){setTimeout(closeNav,100);});
  });
}
...
function closeModal(){
  ...
  if(mCl)mCl.setAttribute('tabindex','-1');
  var mCta=document.querySelector('.modal-cta');
  if(mCta)mCta.setAttribute('tabindex','-1');
...
function openModal(key){
  ...
  if(mCl)mCl.setAttribute('tabindex','0');
  var mCta=document.querySelector('.modal-cta');
  if(mCta)mCta.setAttribute('tabindex','0');
...
document.querySelectorAll('[data-service]').forEach(function(card){
  card.addEventListener('click',function(e){e.stopPropagation();openModal(this.getAttribute('data-service'));});
...
function heroSlide(n){
  ...
  if(d0)d0.style.background=n===0?'rgba(0,95,138,0.8)':'transparent';
  if(d1)d1.style.background=n===1?'rgba(0,95,138,0.8)':'transparent';
