  ...
  link.addEventListener('click',function(e){
    ...
    if(target){
      setTimeout(function(){
        ...
        setTimeout(function(){openModal(key);},400);
      },50);
    }
  });
});

// מיפוי קישורי תפריט לשירותים
var menuServiceMap={
  'אבטחת סייבר':'cyber',
  'ניהול IT':'it',
  'גיבוי וענן':'backup',
  'ניטור 24/7':'monitor',
  'בדיקות חדירה':'pentest',
  'הדרכות אבטחה':'training'
};
...
document.querySelectorAll('a[href^="#"]').forEach(function(link){
  link.addEventListener('click',function(e){
    ...
    var serviceKey=menuServiceMap[text];
    e.preventDefault();
    document.querySelectorAll('.nav-dropdown').forEach(function(d){d.classList.remove('open');});
    if(hbg)closeNav();
    ...
    if(target){
      setTimeout(function(){
        target.scrollIntoView({behavior:'smooth'});
        if(serviceKey){setTimeout(function(){openModal(serviceKey);},400);}
      },50);
    }
  });
});
...
if(bk)bk.addEventListener('click',function(){
  ...
  if(on){var st=document.createElement('style');st.id='hl';st.textContent='a{outline:2px solid #005f8a!important;text-decoration:underline!important;}';document.head.appendChild(st);}
  else{var s=document.getElementById('hl');if(s)s.remove();}
});
